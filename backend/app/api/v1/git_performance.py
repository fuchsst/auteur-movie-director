...
async def get_cached_history(
    ...
    try:
        commits, total = await git_perf.get_history_paginated(
...
async def optimize_repository(
    ...
    try:
        ...
        return {
            "task_id": task.id,
            "status": "queued",
            "message": "Repository optimization queued"
        }
    except ImportError:
        # Fallback to FastAPI background tasks
        background_tasks.add_task(
            git_perf.optimize_repository,
            project_id
        )
        
        return {
            "status": "started",
            "message": "Repository optimization started in background"
        }
...
async def invalidate_cache(
    project_id: str,
    operation: Optional[str] = Query(None, regex="^(status|history|stats|diff)$")
) -> Dict:
    """
...
async def background_commit(
    ...
    try:
        ...
        return {
            "task_id": task.id,
            "status": "queued",
            "message": "Commit operation queued"
        }
    except ImportError:
        # Fallback to FastAPI background tasks
        ...
        return {
            "status": "started",
            "message": "Commit operation started in background"
        }

...
from app.schemas.project import ExportOptions, ExportManifest, ExportStatistics
...
class ProjectExportService:
    ...
    async def export_project(
        ...
        try:
            ...
            await self._copy_project_files(
                project_path, staging_dir / project_id, options
            )
            current_step += 1
            ...
            if options.include_history:
                ...
                await self._bundle_git_repo(
                    project_path, staging_dir / project_id
                )
            current_step += 1
            ...
            await self._update_progress(
                progress_callback, current_step / total_steps,
                "Resolving LFS objects..."
            )
            await self._bundle_lfs_objects(
                project_path, staging_dir / project_id
            )
            current_step += 1
            ...
            manifest = await self._create_manifest(
                project_id, staging_dir / project_id, options
            )
            manifest_path = staging_dir / project_id / "export_manifest.json"
            async with aiofiles.open(manifest_path, 'w') as f:
                await f.write(json.dumps(manifest.dict(), indent=2))
            current_step += 1
    ...
    async def _bundle_git_repo(self, repo_path: Path, staging_path: Path):
        ...
        cmd = ["git", "-C", str(repo_path), "bundle", "create",
               str(bundle_path), "--all"]
    ...
    async def _create_manifest(
        ...
        if (staging_path / "git-bundle.bundle").exists():
            try:
                cmd = ["git", "-C", str(staging_path.parent.parent / project_id),
                       "rev-list", "--count", "HEAD"]
                process = await asyncio.create_subprocess_exec(
                    *cmd,
                    stdout=asyncio.subprocess.PIPE,
    ...
    async def _split_archive(
        ...
        cmd = ["split", "-b", f"{split_size_mb}M", str(archive_path),
               str(archive_path) + ".part"]

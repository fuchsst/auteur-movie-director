...
async def test_complete_project_flow(async_client: AsyncClient, tmp_workspace: Path):
    ...
    async with aiofiles.open(project_json_path) as f:
    ...
    async with aiofiles.open(gitattributes_path) as f:
    ...
    response = await async_client.post(
        f"/api/v1/projects/{project['id']}/assets?category=characters", files=files
    )
    ...
    await redis_client.publish(
        ...
        json.dumps({"type": "project.updated", "data": {"status": "active"}}),
    )
...
async def test_invalid_project_names(async_client: AsyncClient):
    ...
    for name in invalid_names:
        response = await async_client.post(
            "/api/v1/projects", json={"name": name, "quality": "standard"}
        )
...
async def test_concurrent_project_creation(async_client: AsyncClient, tmp_workspace: Path):
    ...
    responses = await asyncio.gather(*[create_project(name) for name in project_names])
...
async def test_websocket_task_execution_flow(async_client: AsyncClient, tmp_workspace: Path):
    ...
    response = await async_client.post(
        "/api/v1/projects", json={"name": "task-execution-test", "quality": "high"}
    )
    assert response.status_code == 201
    project = response.json()
    ...
    response = await async_client.post(f"/api/v1/projects/{project['id']}/tasks", json=task_data)
...
async def test_character_asset_lifecycle(async_client: AsyncClient, tmp_workspace: Path):
    ...
    # 1. Create project
    response = await async_client.post(
        "/api/v1/projects", json={"name": "character-lifecycle-test", "quality": "standard"}
    )
    assert response.status_code == 201
    project = response.json()
    ...
    response = await async_client.post(
        f"/api/v1/projects/{project['id']}/characters", json=character_data
    )
    assert response.status_code == 201
    ...
    response = await async_client.post(
        f"/api/v1/projects/{project['id']}/characters/{character['id']}/base-face", files=files
    )
    assert response.status_code == 200
    ...
    response = await async_client.get(f"/api/v1/projects/{project['id']}/characters")
    assert response.status_code == 200
...
async def test_quality_mapping_parameters(async_client: AsyncClient):
    ...
    quality_mappings = {
        "low": {"steps": 20, "cfg_scale": 7.0, "sampler": "euler", "target_vram": 12},
        "standard": {"steps": 30, "cfg_scale": 7.5, "sampler": "euler_a", "target_vram": 16},
        "high": {"steps": 50, "cfg_scale": 7.5, "sampler": "dpm++_2m", "target_vram": 24},
    }

    for quality, expected_params in quality_mappings.items():
        # Create project with specific quality
        response = await async_client.post(
            "/api/v1/projects", json={"name": f"quality-test-{quality}", "quality": quality}
        )
        assert response.status_code == 201
        project = response.json()
        ...
        response = await async_client.post(
            ...
            json={"type": "text_to_image", "prompt": "Test prompt", "node_id": f"node-{quality}"},
        )
        assert response.status_code == 202
        task = response.json()
...
async def test_structure_enforcement(async_client: AsyncClient, tmp_workspace: Path):
    ...
    # Create project
    response = await async_client.post(
        "/api/v1/projects", json={"name": "structure-test", "quality": "standard"}
    )
    assert response.status_code == 201
    project = response.json()
...
async def test_takes_system_integration(async_client: AsyncClient, tmp_workspace: Path):
    ...
    # Create a take
    take_data = {"shot_id": "S01_S01", "description": "First take of opening shot"}

    response = await async_client.post(
        f"/api/v1/projects/{project['id']}/shots/S01_S01/takes", json=take_data
    )
    assert response.status_code == 201
    take = response.json()
    ...
    project_path = workspace_service.get_project_path(project["name"])
    take_dir = (
        project_path / "03_Renders" / "Chapter_01" / "Scene_01" / "Shot_01" / "takes" / "take_001"
    )

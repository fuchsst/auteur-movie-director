...
from app.services.git_performance import PERFORMANCE_THRESHOLDS, GitPerformanceManager
...
class TestGitPerformanceLoad:
    ...
    async def test_history_fetch_performance(self, large_repo):
        ...
        with patch.object(perf_manager.git_service, "_get_project_path", return_value=large_repo):
            ...
            start = time.time()
            commits, total = await perf_manager.get_history_paginated(project_id, page=1, limit=50)
            elapsed_ms = (time.time() - start) * 1000
    ...
    async def test_memory_usage(self, large_repo):
        ...
        with patch.object(perf_manager.git_service, "_get_project_path", return_value=large_repo):
            ...
            for page in range(1, 5):
                await perf_manager.get_history_paginated(project_id, page=page, limit=50)
